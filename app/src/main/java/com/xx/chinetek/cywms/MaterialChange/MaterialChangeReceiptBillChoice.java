...
public class MaterialChangeReceiptBillChoice extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener {
        ...
        if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_UP)// 如果为Enter键
        {
            if(receiptModels!=null && receiptModels.size()>0) {
                ...
                if (index!=-1) {
                    ...
                    LogUtil.WriteLog(MaterialChangeReceiptBillChoice.class, TAG_GetT_PalletDetailByBarCode, code);
    ...
    void AnalysisGetT_PalletDetailByBarCodeJson(String result) {
        LogUtil.WriteLog(MaterialChangeReceiptBillChoice.class, TAG_GetT_PalletDetailByBarCode, result);
        try {
